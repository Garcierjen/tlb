...
class ServerMonitor {
  ...
  async fetchStatus() {
    ...
    try {
      ...
      if (data.online) {
        ...
        if (playersOnline)
          playersOnline.textContent = String(data.players.online);
        if (playersMax)
          playersMax.textContent = String(data.players.max);
        if (motd)
          motd.textContent = data.motd.join(" ");
        if (list) {
          list.innerHTML = data.players.list.length > 0 ? data.players.list.map((p) => `<span class="player-tag">${this.escHtml(p)}</span>`).join("") : '<span class="no-players">no players online</span>';
        }
      } else {
        this.setOffline();
      }
  ...
  initCopyHandlers() {
    document.addEventListener("click", async (e) => {
      const target = e.target.closest(".ipcopy");
      if (!target)
        return;
